  ...
  register(api) {
    ...
    api.registerTool({
      ...
      async execute(_id: any, params: any) {
        ...
        const text = list.map((n: any) => `[${n.priority}] ${n.title}: ${n.content}`).join("\n") || "No notifications";
        return { 
    ...
    api.registerTool({
      ...
      parameters: { type: "object", properties: {} },
      async execute() {
        const upcoming = reminders.filter((r: any) => !r.triggered);
        const text = upcoming.map((r: any) => `- ${r.title} (${r.triggerTime.toLocaleString()})`).join("\n") || "No upcoming reminders";
        return { 
          content: [{ type: "text", text }],
